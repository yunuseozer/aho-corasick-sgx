...
pub type TestCollection = &'static [&'static [SearchTest]];
...
macro_rules! testconfig {
    (overlapping, $name:ident, $collection:expr, $kind:ident, $with:expr) => {
        //#[test]
        pub fn $name() {
            use crate::tests::run_search_tests;
...
testcombo!(search_leftmost_longest, crate::tests::AC_LEFTMOST_LONGEST, LeftmostLongest);
testcombo!(search_leftmost_first, crate::tests::AC_LEFTMOST_FIRST, LeftmostFirst);
testcombo!(
    search_standard_nonoverlapping, crate::tests::AC_STANDARD_NON_OVERLAPPING, Standard
);
...
//#[test]
pub fn search_tests_have_unique_names() {
...
//#[test]
//#[should_panic]
pub fn stream_not_allowed_leftmost_first() {
    let fsm = AhoCorasickBuilder::new()
        .match_kind(MatchKind::LeftmostFirst)
        .build(None::<String>);
    should_panic!(assert_eq!(fsm.stream_find_iter(&b""[..]).count(), 0));
}

//#[test]
//#[should_panic]
pub fn stream_not_allowed_leftmost_longest() {
    let fsm = AhoCorasickBuilder::new()
...
//#[test]
//#[should_panic]
pub fn overlapping_not_allowed_leftmost_first() {
    let fsm = AhoCorasickBuilder::new()
        .match_kind(MatchKind::LeftmostFirst)
        .build(None::<String>);
    should_panic!(assert_eq!(fsm.find_overlapping_iter("").count(), 0));
}

//#[test]
//#[should_panic]
pub fn overlapping_not_allowed_leftmost_longest() {
    let fsm = AhoCorasickBuilder::new()
        .match_kind(MatchKind::LeftmostLongest)
        .build(None::<String>);
    should_panic!(assert_eq!(fsm.find_overlapping_iter("").count(), 0));
